...
class DipoleLayer(nn.Module):
    def __init__(
        ...
        self.brnn = nn.GRU(feature_size, feature_size, batch_first=True, bidirectional=True)
        self.alpha_li = nn.Linear(feature_size * 2, 1)

        self.combination_mapping = nn.Sequential(
            nn.Linear(feature_size * 4, feature_size),
            nn.Tanh(),
        )
    
    def compute_alpha(self, h, lengths):
        """Computes alpha attention."""
        h, _ = rnn_utils.pad_packed_sequence(h, batch_first=True)
        attn_alpha = self.alpha_li(h)
        attn_alpha = [torch.softmax(attn_alpha[i, :lengths[i]], dim=0) for i in range(len(lengths))]
        attn_alpha = rnn_utils.pad_sequence(attn_alpha, batch_first=True)
        return attn_alpha
    ...
    def forward(self, x, mask=None):
        ...
        h, h_bar = self.brnn(x) # h: (patient, sequence len, feature_size * 2), h_bar: (1 * 2, patient, feature_size)
        h_bar = h_bar.permute(1, 0, 2).reshape(batch_size, -1)
        attn_alpha = self.compute_alpha(h, lengths)
        h, _ = rnn_utils.pad_packed_sequence(h, batch_first=True)
        c = torch.sum(attn_alpha * h, dim=1)
        c = self.combination_mapping(torch.cat([c, h_bar], dim=-1))
        return c
